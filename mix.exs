defmodule ExampleMix.MixProject do
  use Mix.Project

  def project do
    [
      app: :example_mix,
      version: "0.1.0",
      elixir: "~> 1.14",
      start_permanent: Mix.env() == :prod,
      deps: deps()
    ]
  end

  # Run "mix help compile.app" to learn about applications.
  def application do
    [
      # Mod define the entry point to trigger when we command mix or mix run because by default it only triggers
      # when compiling a new component or if there's a change in older one
      mod: {ExampleMix, []},
      extra_applications: [:logger]
    ]
  end

  # Run "mix help deps" to learn about dependencies.
  defp deps do
    [
      # This is how we add dependencies in Elixir and after adding here we call "mix deps.get" to add these dependencies into project
      {:uuid, "~> 1.1"}
      # {:dep_from_hexpm, "~> 0.3.0"},
      # {:dep_from_git, git: "https://github.com/elixir-lang/my_dep.git", tag: "0.1.0"}
    ]
  end
end
